...
fetch_entry() {
    ...
    if [ "$queries_only" != "true" ]; then
        ...
        local saved_scanner=""
        if [ "$skip_scanner" = "true" ] && [ -f "$out_src/scanner.c" ]; then
            saved_scanner=$(mktemp)
            cp "$out_src/scanner.c" "$saved_scanner"
        fi

        # Copy only files needed for compilation (parser.c is generated by CI)
        rm -rf "$out_src"
        mkdir -p "$out_src"
        if [ "$skip_scanner" = "true" ] && [ -n "$saved_scanner" ]; then
            # Restore our maintained scanner
            cp "$saved_scanner" "$out_src/scanner.c"
            rm -f "$saved_scanner"
        else
